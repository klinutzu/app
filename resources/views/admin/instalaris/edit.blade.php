...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.instalari.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.instalaris.update", [$instalari->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('numar_comanda') ? 'is-invalid' : '' }}" name="numar_comanda_id" id="numar_comanda_id" required>
                    @foreach($numar_comandas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('numar_comanda_id') ? old('numar_comanda_id') : $instalari->numar_comanda->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('ip') ? 'is-invalid' : '' }}" type="text" name="ip" id="ip" value="{{ old('ip', $instalari->ip) }}">
                @if($errors->has('ip'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ip') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('user') ? 'is-invalid' : '' }}" type="text" name="user" id="user" value="{{ old('user', $instalari->user) }}">
                @if($errors->has('user'))
                    <div class="invalid-feedback">
                        {{ $errors->first('user') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('parola') ? 'is-invalid' : '' }}" type="text" name="parola" id="parola" value="{{ old('parola', $instalari->parola) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('survey') ? 'is-invalid' : '' }}" name="survey_id" id="survey_id" required>
                    @foreach($surveys as $id => $entry)
                        <option value="{{ $id }}" {{ (old('survey_id') ? old('survey_id') : $instalari->survey->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
