...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.detaliitehnice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.detaliitehnices.update", [$detaliitehnice->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('numar_comanda') ? 'is-invalid' : '' }}" name="numar_comanda_id" id="numar_comanda_id" required>
                    @foreach($numar_comandas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('numar_comanda_id') ? old('numar_comanda_id') : $detaliitehnice->numar_comanda->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('detalii_tehnice') ? 'is-invalid' : '' }}" name="detalii_tehnice_id" id="detalii_tehnice_id" required>
                    @foreach($detalii_tehnices as $id => $entry)
                        <option value="{{ $id }}" {{ (old('detalii_tehnice_id') ? old('detalii_tehnice_id') : $detaliitehnice->detalii_tehnice->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('link_monitorizare') ? 'is-invalid' : '' }}" type="text" name="link_monitorizare" id="link_monitorizare" value="{{ old('link_monitorizare', $detaliitehnice->link_monitorizare) }}">
